...
export default function VerifyOtpClient() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-900 to-slate-800 px-4 relative text-slate-100">
      ...
      {paramOtp && (
        <div className="mb-6 w-full max-w-md bg-slate-800 text-white p-4 rounded-2xl shadow-2xl border border-blue-500/50 flex items-center justify-between gap-4 animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-blue-600 rounded-xl text-white">
              <KeyRound className="w-5 h-5" />
            </div>
            <div>
              <p className="text-[11px] font-medium text-slate-300">Demo Admin / CB OTP:</p>
          ...
          <button
            ...
            className="px-3.5 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all shadow-md"
          >
            {copied ? <Check className="w-3.5 h-3.5 text-emerald-300" /> : null}
      ...
      <div className="w-full max-w-md bg-white text-slate-900 rounded-2xl shadow-2xl border border-slate-100 p-8">
        ...
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-bold text-slate-900 mt-3">
            Admin / Warehouse Security Check
          </h2>

          <p className="text-sm text-slate-500 mt-1">
            Enter your 6-digit authentication OTP
          </p>
        </div>
        ...
        <form onSubmit={handleVerify} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-slate-900 hover:bg-slate-800 text-white py-3 rounded-xl font-semibold flex items-center justify-center gap-2 transition-all disabled:opacity-50"
          >
            {loading ? "Verifying..." : "Verify OTP & Access Panel"}
